...
int Dude::get_x() const
{
	return x;
}

int Dude::get_y() const
{
	return y;
}

void Dude::GetInput(const MainWindow& wnd)
{
...
void Dude::UpdateX()
{
	if (x < 0)
		x = 0;
	else if (x + dudeWidth >= Graphics::ScreenWidth)
...
void Dude::UpdateY()
{
	if (y <= 0)
		y = 0;
	else if (y + dudeHeight >= Graphics::ScreenHeight)
		y = (Graphics::ScreenHeight - 1) - dudeHeight;
